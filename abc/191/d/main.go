...
func main() {
	...
	x := abs(int(math.Round(readFloat64() * 10000)))
	y := abs(int(math.Round(readFloat64() * 10000)))
	r := int(math.Round(readFloat64() * 10000))
	debug.println(x, y, r)

	ans := 0
	s := x + r - (x+r)%10000
	for xx := s; xx >= x-r; xx -= 10000 {
		xd := xx - x
		yy := int(math.Sqrt(float64(r*r - xd*xd)))
		if xd != 0 && yy == r {
			yy--
		}
		up, down := y+yy, y-yy
		up, down = int(math.Floor(float64(up)/10000)), int(math.Ceil(float64(down)/10000))
		ans += up - down + 1
	}
	fmt.Println(ans)
}
