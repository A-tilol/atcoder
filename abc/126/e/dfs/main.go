...
var a map[int]struct{}
var xy map[int]map[int]struct{}

func main() {
	n := readInt()
	m := readInt()
	xy = make(map[int]map[int]struct{})
	for i := 0; i < m; i++ {
		x := readInt()
		y := readInt()
		readInt()
		if _, ok := xy[x-1]; !ok {
			xy[x-1] = make(map[int]struct{})
		}
		if _, ok := xy[y-1]; !ok {
			xy[y-1] = make(map[int]struct{})
		}
		xy[x-1][y-1] = struct{}{}
		xy[y-1][x-1] = struct{}{}
	}
